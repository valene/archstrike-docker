...
# modified from official archlinux for archstrike
# requires root
set -e
...
ROOTFS=$(mktemp -d ${TMPDIR:-/var/tmp}/rootfs-archstrike-XXXXXXXXXX)
chmod 755 $ROOTFS
...
case "$arch" in
	...
	*)
		...
		PACMAN_MIRROR_ARCHSTRIKE='Server = https://mirror.archstrike.org/$arch/$repo'
		PACMAN_MIRRORLIST_ARCHSTRIKE='Include = /etc/pacman.d/archstrike-mirrorlist'
		PACMAN_EXTRA_PKGS=''
		EXPECT_TIMEOUT=60
		ARCH_KEYRING=archlinux
		ARCHSTRIKE_KEY='9D5F1C051D146843CDA4858BDE64825E7CBC0D51'
		DOCKER_IMAGE_NAME=archstrike-personal
		;;
esac

export PACMAN_MIRRORLIST
export PACMAN_MIRROR_ARCHSTRIKE
...
arch-chroot $ROOTFS /bin/sh -c 'echo -e "[archstrike]\n" >> /etc/pacman.conf'
arch-chroot $ROOTFS /bin/sh -c 'echo $PACMAN_MIRROR_ARCHSTRIKE >> /etc/pacman.conf'
arch-chroot $ROOTFS /bin/sh -c "pacman -Syy; pacman-key --init; dirmngr < /dev/null ; pacman-key -r $ARCHSTRIKE_KEY; pacman-key --lsign-key $ARCHSTRIKE_KEY" 
arch-chroot $ROOTFS /bin/sh -c "yes | pacman -S archstrike-keyring ; yes | pacman -S archstrike-mirrorlist"
arch-chroot $ROOTFS /bin/sh -c "sed -i 's/Server\W=\Whttps:\/\/mirror.archstrike.org\/\$arch\/\$repo/Include = \/etc\/pacman.d\/archstrike-mirrorlist/g' /etc/pacman.conf"
arch-chroot $ROOTFS /bin/sh -c "pacman -Syy; pkill gpg-agent"
